...
import { AppShell, CommandBar } from './components/dashboard-shell';
import { CoreActionBoard } from './components/dashboard/CoreActionBoard';
import { PortfolioHealthBoard } from './components/dashboard/PortfolioHealthBoard';
...
export default function App() {
  ...
  const navigationGroups = APP_SHELL_NAV_GROUPS;
  const pageMeta = APP_SHELL_PAGE_META;
  const {
    primaryItems: mobilePrimaryNavItems,
    moreItems: mobileMoreNavItems,
    moreActive: mobileMoreActive,
  } = getMobileAppShellNav(activeTab);
  ...
  return (
    <>
      <AppShell
        ...
        commandBar={(
          <CommandBar>
            <div className="app-shell__command-bar-layout">
              <div className="app-shell__page-meta">
                <div className="app-shell__page-meta-label">PulsePort command deck</div>
                <div className="app-shell__page-meta-copy">
                  <div className="app-shell__page-meta-title">
                    {pageMeta[activeTab].title}
                  </div>
                  <div className="app-shell__page-meta-subtitle">
                    {pageMeta[activeTab].subtitle}
                  </div>
                </div>
                <div className="app-shell__page-meta-status hidden sm:flex items-center gap-2">
                  <div className={`status-dot ${lastUpdated ? 'status-dot-live' : ''}`} />
                  {lastUpdated && (
                    <span className="app-shell__page-meta-status-copy">
              ...
              <div className="app-shell__command-bar-actions app-header-actions">
                <button
              ...
              <motion.div key="home" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="premium-home">
                <section className="premium-home-hero">
                  ...
                  >
                    <CoreActionBoard
                      summary={summary}
                      topHolding={frontPagePortfolioRows[0]}
                      hasInvestmentHistory={summary.netInvestment > MIN_INVESTMENT_THRESHOLD}
                      onOpenInvestments={() => setActiveTab('pulsechain-official')}
                      onOpenTransactions={() => setActiveTab('history')}
                      onOpenProfitPlanner={() => setProfitPlannerOpen(true)}
                    />
                    <div
                      style={{
                        display: 'grid',
                    ...
                    >
                      <PortfolioHealthBoard
                        summary={summary}
                        topHoldings={frontPagePortfolioRows}
                        chainRows={frontPageChainRows}
