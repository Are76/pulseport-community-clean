...
export interface TransactionListProps {
  ...
  /** Visual treatment for the transaction surface */
...
export function TransactionList({
  ...
  if (visible.length === 0) {
    return <div className={`tx-list-empty${isExecutiveSurface ? ' tx-list-empty--executive' : ''}`}>{emptyMessage}</div>;
  }

  return (
    <div className={`tx-list${isExecutiveSurface ? ' tx-list--executive' : ''}`}>
      {transactions.map(tx => {
        ...
        const triggerLabel = `${isExpanded ? 'Collapse' : 'Expand'} transaction ${tx.hash}`;

        return (
          <div
            key={tx.id}
            className={`tx-card-row${isHidden ? ' tx-card-row--hidden' : ''}${isExecutiveSurface ? ' tx-card-row--executive' : ''}`}
            style={{ borderBottom: '1px solid var(--border)' }}
          >
            {/* -- Card row ----------------------------------------------- */}
            <div
              className={`tx-card${compact ? ' tx-card--compact' : ''}${isExecutiveSurface ? ' tx-card--executive' : ''}`}
